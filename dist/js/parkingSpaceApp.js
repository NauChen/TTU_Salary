...
$(function () {
  $('#parkingSpaceApplicationList').DataTable(_objectSpread(_objectSpread({}, commonSettingsTable), {}, {
  ...
  $('#parkingSpaceDetailModel').on('show.bs.modal', function (event) {
    ...
    // 解除先前綁定的點擊事件
    $('#updateBtn').off('click');
    var parkingSpaceData = dataset_parkingSpaceApp.find(function (perkingSpace) {
    ...
    if (parkingSpaceData) {
      ...
      if (parkingSpaceData.status != '已收件，待收費') {
