...
// 付款資訊-合格廠商
var companyData = ["超級棒股份有限公司", "大富翁科技有限公司", "迅捷物流股份有限公司", "綠能科技有限公司", "全球股份有限公司", "宇宙科技有限公司", "光速科技有限公司"];
// 付款資訊-可用汽車位
var parkingSpaceCarData = ["B1-01", "B1-02", "B2-03", "B3-04", "B2-05", "B2-07", "B3-11"];
// 付款資訊-可用機車位
var parkingSpaceMotoData = ["B1-21", "B1-22", "B2-23", "B3-24", "B2-25", "B2-26", "B3-27"];
// 付款資訊-已發識別證
var idCardData = ["N000030", "N000051", "N000040", "N000089", "N000012", "N000060", "N000070"];
// 付款資訊-該合格廠商所擁有的培育室(若辦不到就用開放的培育室吧)
var roomData = ["新德惠大樓/101", "綜合大樓/201", "挺生大樓/714", "實驗大樓/101", "北設工大樓/713", "綜合大樓/A4-102", "挺生大樓/809"];
// 付款資訊-支付目的
...
$(function () {
  ...
  $('#confirmPaymentCultivationModel').on('show.bs.modal', function (event) {
    ...
    var roomThisId = String(button.data('id'));
    console.log('roomId 1 :', roomThisId); // 確認 roomId 是否正確獲取

    if (!roomThisId.endsWith('_N')) {
      // 表示為已配置的紀錄id
      var openRoomData = dataset_confirmPaymentCultivationRoom.find(function (thisData) {
        return thisData.id === roomThisId;
      });
      if (openRoomData) {
      ...
      var _openRoomData = dataset_confirmPaymentCultivationRoom.find(function (room) {
        return room.roomId === roomId_N;
      });
      if (_openRoomData) {
        $('#cultivation_deleteData').addClass('d-none');
        $('#cultivation_building').text(_openRoomData.building);
        $('#cultivation_room').text(_openRoomData.room);
        $('#cultivation_squareMeters').text(_openRoomData.squareMeters);
        $('#cultivation_startDate').text('');
        $('#cultivation_endDate').text('');
        $('#cultivation_rate').text(_openRoomData.amount); // 修改金額不會回存培育室預設的數字，避免兩個期數不同價格，資料庫混亂。
    ...
    $('#cultivation_deleteData').on('click', function () {
      ...
      swalConfirm('確定要刪除嗎？', '是啊，這個沒用了...', '不要阿 ~ 我是誤按！！', function () {
  ...
  // 車位
  $('#confirmPaymentParkingSpaceList').DataTable(_objectSpread(_objectSpread({}, commonSettingsTable), {}, {
  ...
  // 培育室配置廠商-增加緊急日期
  dataset_confirmPaymentCultivationRoom.forEach(function (item) {
    if (!item.company) {
  ...
  $('#confirmPaymentCultivationRoomList').DataTable(_objectSpread(_objectSpread({}, commonSettingsTable), {}, {
    ...
    // 2
    {
      ...
      render: function render(data, type, row) {
        var receivedDate1 = row.receivedDate1;
        if (!receivedDate1) {
          if (data) {
      ...
      // 5
      render: function render(data, type, row) {
        var receivedDate2 = row.receivedDate2;
        if (!receivedDate2) {
          if (data) {
            return '<i class="fa-regular fa-calendar-days"></i> ' + data;
          } else {
      ...
      // 6
      render: function render(data, type, row) {
        var receivedDate3 = row.receivedDate3;
        if (!receivedDate3) {
          if (data) {
            return '<i class="fa-regular fa-calendar-days"></i> ' + data;
          } else {
      ...
      // 7
      render: function render(data, type, row) {
        var receivedDate4 = row.receivedDate4;
        if (!receivedDate4) {
          if (data) {
            return '<i class="fa-regular fa-calendar-days"></i> ' + data;
          } else {
  ...
  // 收款紀錄與收據-表格
  $('#confirmPaymentAllList').DataTable(_objectSpread(_objectSpread({}, commonSettingsTable), {}, {
    ...
    // 2
    {
      ...
      // 3
      render: function render(data, type, row) {
        var placeOrPrint = row.placeOrPrint;
        if (placeOrPrint) {
...
$(function () {
  ...
  $('#confirmPaymentModel').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget);
    var paymentId = String(button.data('id'));
    ...
    var paymentData = dataset_confirmPaymentAll.find(function (payment) {
