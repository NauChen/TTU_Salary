...
$(function () {
  ...
  var table = $('#progressCheckList').DataTable(_objectSpread(_objectSpread({}, commonSettingsTable), {}, {
    ...
    // 3
    {
      ...
      render: function render(data, type, row) {
        var isReprint = row.isReprint;
        var isRenew = row.isRenew;
        if (isReprint) {
          return isReprint;
        } else if (isRenew) {
          return isRenew + '：' + data;
  ...
  $('.uploadRemittance, .reuploadRemittance').click(function (event) {
    ...
    var button = $(this);
    var progressId = button.data('id'); // 獲取按鈕的 data-id 屬性

    if (progressId) {
      var progressData = dataset_progressCheck.find(function (progress) {
