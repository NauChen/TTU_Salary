...
$(function () {
  $(document).on("click", ".renew-btn", function (event) {
    ...
    var renewId = $(this).data('id');
    var renewLink = './parkingSpaceRenew.html?id=' + renewId;
    swalConfirmNoToastSandId('登記使用人是否變更呢?', '是', '否', function () {
      window.location.href = renewLink;
    }, '已將收款帳戶資訊發至貴司的主要聯絡信箱。<br><br>前往「進度查詢」：<br>可上傳匯款憑證，或是再次發送大同大學帳戶資訊。', 'top', renewId);
  });
});